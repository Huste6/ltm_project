...
void handle_save_practice_answer(Server *server, ClientSession *client, Message *msg)
{
    ...
    if (msg->param_count < 3)
    {
        send_error_or_response(client->socket_fd, CODE_SYNTAX_ERROR,
                               "Usage: SAVE_PRACTICE_ANSWER practice_id|question_index|answer");
        return;
    }
    ...
    if (elapsed > time_limit_seconds)
    {
        ...
        db_log_activity(server->db, "WARNING", client->username,
                        "SAVE_PRACTICE_ANSWER", "Time expired");
        return;
    }
...
void handle_submit_practice(Server *server, ClientSession *client, Message *msg)
{
    ...
    // Validate params: now only practice_id (answers in buffer)
    if (msg->param_count < 1)
    {
        send_error_or_response(client->socket_fd, CODE_SYNTAX_ERROR,
                               "Usage: SUBMIT_PRACTICE practice_id");
        return;
    }
