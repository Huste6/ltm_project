...
(
    ...
    status ENUM
                ('NOT_STARTED', 'IN_PROGRESS', 'FINISHED') DEFAULT 'NOT_STARTED',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    INDEX idx_practice_id
                                    (practice_id),
    FOREIGN KEY
                                    (practice_id) REFERENCES practice_sessions
                                    (practice_id) ON
                                    DELETE CASCADE,
    FOREIGN KEY (question_id)
                                    REFERENCES questions
    ...
    INDEX idx_level
                                        ...
                                        -- View để dễ query
                                        CREATE VIEW active_sessions_view
                                        AS
                                            SELECT s.session_id, s.username, s.login_time, s.last_activity
                                            FROM sessions s
                                            WHERE s.is_active = 1;

                                        CREATE VIEW room_leaderboard_view
                                        AS
                                            SELECT
                                                r.room_id,
                                                r.room_name,
    ...
    IN p_room_id VARCHAR
                                        (32),
    IN p_room_name VARCHAR
                                        (100),
    IN p_creator VARCHAR
                                        (20),
    ...
    IN p_time_limit INT
                                        ...
                                        -- Insert room
                                        INSERT INTO rooms
                                            (room_id, room_name, creator, num_questions, time_limit_minutes)
                                        VALUES
                                            (p_room_id, p_room_name, p_creator, p_num_questions, p_time_limit);

                                        -- Random chọn câu hỏi
                                        INSERT INTO room_questions
                                            (room_id, question_id, question_order)
    ...
    VALUES
                                        ...
                                        CREATE PROCEDURE get_leaderboard(IN p_room_id VARCHAR
                                        (32))
                                        BEGIN
                                            SELECT
                                                username,
